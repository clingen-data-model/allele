...
When creating a central repository for genomic variants, it is critical to ensure that any representation of the same allele be recognized as such. An allele may be represented multiple ways for a variety of reasons such as use of historical versus standardized nomenclature or use of different reference sequences.  The ClinGen Allele Data Model v0.1 addresses this by tying all representation of an allele, referred to as [simple alleles](/source/allele/conceptual/simple_allele/index.html), to a single identifier, referred to as the [CanonicalAllele](/allele/conceptual/canonical_allele/index.html).
